...
`components/insight.py` renders a camera capture widget (`st.camera_input`) — tap it to take a photo with your device's camera, which uploads straight to Supabase Storage (overwriting the previous photo). The card displays both the photo's capture time and the latest sensor reading's time (converted to PH time) along with the actual sensor values, so you can see at a glance what the AI is basing its assessment on. `services/ai_insights.py` then sends the photo, the current sensor reading, and a recent historical min/max/avg trend for each metric to Gemini (`gemini-3.1-flash-lite`, via the REST **Interactions API**: `POST https://generativelanguage.googleapis.com/v1beta/interactions`), with a fixed "chilli-pepper cultivation expert" instruction prompt. The model returns a short, plain-language condition summary and one actionable tip, shown at the top of the dashboard, before the current environmental conditions.

The prompt deliberately separates roles: the **photo** is used only to describe visual appearance (leaf color, wilting, pests) since it may be stale, while the **current reading + historical trend** drive the actual environmental assessment (e.g. "soil moisture has been fluctuating and is now critically dry"). The photo and the sensor reading are captured independently (the photo whenever you choose to take one, the sensor reading every 5 minutes automatically) and usually won't share the exact same timestamp — the prompt instructs Gemini to note when they're more than ~15 minutes apart so the assessment doesn't silently assume a stale photo reflects current conditions.
